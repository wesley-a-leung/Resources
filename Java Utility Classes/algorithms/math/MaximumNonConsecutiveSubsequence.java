...
public class MaximumNonConsecutiveSubsequence {
    ...
    public static int solve(int[] sequence) {
        ...
        dp[1] = Math.max(dp[0], sequence[1]); // if there are 2 values, then the maximum of the 2 is the solution
        for (int i = 2; i < sequence.length; i++) { // we start at 2 since 0 and 1 have been solved
            dp[i] = Math.max(dp[i-2] + sequence[i], dp[i-1]);    // the solutions is the maximum of either the value at index i
