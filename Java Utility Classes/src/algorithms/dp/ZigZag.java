...
public class ZigZag {
	...
	public static int solve(int[] sequence) {
		int[][] solutions = new int[2][sequence.length];	// the 2d array that stores the solution of the maximum subsequence ending with index i
		...
		solutions[0][0] = 1; // the initial lengths must be at least 1 since a single number is considered a zig-zag sequence
		solutions[1][0] = 1;
		for (int i = 1; i < sequence.length; i++) { // iterating through every number in the sequence to find the maximum subsequence ending with that index
			...
			if (sequence[i] - sequence[i-1] > 0) { // positive difference
				solutions[0][i] = Integer.max(solutions[1][i-1] + 1, solutions[0][i-1]);	// the length of the subsequence is the maximum of either the 
																						// negative solution + 1 (since signs need to alternate), or
																						// the current positive solution
				solutions[1][i] = solutions[1][i-1];									// the current negative solution remains the same
			} else if (sequence[i] - sequence[i-1] < 0) {	// negative difference
				solutions[1][i] = Integer.max(solutions[0][i-1] + 1, solutions[1][i-1]);	// the length of the subsequence is the maximum of either the 
				 																		// positive solution + 1 (since signs need to alternate), or 
																						// the current negative solution
				solutions[0][i] = solutions[0][i-1];									// the current positive solution remains the same
				...
				solutions[0][i] = solutions[0][i-1];
				solutions[1][i] = solutions[1][i-1];
			} // if else
		} // for i
		return Math.max(solutions[0][sequence.length-1], solutions[1][sequence.length-1]);	// the longest solution is the max subsequence that ends
