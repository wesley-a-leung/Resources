...
// Time Complexity:
//   constructor: O(N)
//   makeRoot, findRoot, findParent, lca, link, cut, updateVertex, queryPath: O(log N)
...
struct LinkCutTree {
    vector<Node> T;
    LinkCutTree(int N, const Data &vdef) { T.reserve(N); for (int i = 0; i < N; i++) T.emplace_back(i, vdef); }
