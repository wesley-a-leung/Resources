...
template <const int MAXV, class unit, class Comparator = less<unit>> struct DAGSP {
    Comparator cmp; unit INF, dist[MAXV]; pair<int, unit> to[MAXV]; vector<pair<int, unit>> adj[MAXV]; TopologicalOrder<MAXV> ord;
    DAGSP(unit INF) : INF(INF) {}
    ...
    void run(int V, const vector<int> &src = vector<int>()) { // if src is empty, then all vertices are sources
        fill(dist, dist + V, src.empty() ? 0 : INF); fill(to, to + V, make_pair(-1, 0)); ord.run(V);
