...
#include "../../datastructures/unionfind/UnionFindSemiPersistent.h"
using namespace std;
...
//     of the list
// Functions:
//   find(t, v): inherited from UnionFindSemiPersistent, finds an arbitrary
//     root of the 2-edge connected component containing vertex v, after the
//     edge at index t in edges is added
//   connected(t, v, w): inherited from UnionFindSemiPersistent,
...
//   getSize(t, v): inherited from UnionFindSemiPersistent, returns the size of
//     the 2-edge connected component containing vertex v, after the
//     edge at index t in edges is added
//   getCnt(t): inherited from UnionFindSemiPersistent, returns the number of
//     2-edge connected components, after the edge at index t in edges is added
//   getFirst(v, w): inherited from UnionFindSemiPersistent, returns the index
//     of the first edge in edges that vertices v and w are in the same
//     2-edge connected component, -1 if v == w and curTime + 1 if they are
//     never in the same 2-edge connected component
...
struct SemiDynamicBridgesSemiPersistent : public UnionFindSemiPersistent {
  vector<int> bridgeCnt;
  SemiDynamicBridgesSemiPersistent(int V, const vector<pair<int, int>> &edges)
      : UnionFindSemiPersistent(V), bridgeCnt(edges.size()) {
