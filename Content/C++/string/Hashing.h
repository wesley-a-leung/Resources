...
// Computes the hash of an array/string to allow for easy computation of
//   substring hashes
// Template Arguments:
...
//   getHash(l, r): returns a type HASH with the hashes in the substring [l, r)
//   merge(h1, h2): returns a type HASH of 2 hashes merged together as if
//     the substring represented by h2 was immediately after h1
//   concat(l1, r1, l2, r2): returns a type HASH of
//     the substring [l1, r1) contactenated with the substring [l2, r2)
...
template <class T, class HTYPE, const int HASHES> struct Hashing {
  ...
  template <class It>
  Hashing(It st, It en, T offset, const arr &BASE, const arr &MOD)
      : N(en - st), BASE(BASE), MOD(MOD), H(N + 1), POW(N + 1) {
    ...
    for (int i = N - 1; i >= 0; i--) for (int h = 0; h < HASHES; h++)
      H[i][h] = addMod(mulMod(H[i + 1][h], BASE[h], MOD[h]),
                       posMod(HTYPE(st[i] - offset) + HTYPE(1), MOD[h]),
                       MOD[h]);
  }
  ...
  template <class It> Hashing(It st, It en, T offset, const arr &MOD)
      : Hashing(st, en, offset, initBase(MOD), MOD) {}
