...
int extremeVertex(const Polygon &poly, ref dir) {
    ...
    auto cmp = [&] (int i, int j) { return sgn(cross(pp, poly[i % n] - poly[j % n])); };
...
pair<int, int> lineConvexPolyInter(const Polygon &poly, const Line &line) {
    ...
    for (int i = 0; i < 2; i++) {
        int lo = endB, hi = endA;
        while ((lo + 1) % n != hi) { int m = ((lo + hi + (lo < hi ? 0 : n)) / 2) % n; (cmpL(m) == cmpL(endB) ? lo : hi) = m; }
        (i == 0 ? ret.first : ret.second) = (lo + !cmpL(hi)) % n; swap(endA, endB);
    }
