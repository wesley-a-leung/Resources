...
// API is similar to unordered_set and unordered_map in C++ 03, except there is no count function (use member function find instead)
// In practice, has a large constant, but faster than unordered_set and unordered_map (at the expense of using more memory)
...
// There are additional functions to find the kth element with find_by_order(k) and find the 0-indexed rank of a key with order_of_key(key)
// In practice, has a moderate constant, slower that set and map as well as custom written balanced binary search trees
