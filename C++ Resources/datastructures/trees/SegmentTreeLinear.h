...
template <const int MAXN> struct SegmentTreeLinear {
    ...
    Data query(int cur, int cL, int cR, int l, int r) {
        ...
        int m = cL + (cR - cL) / 2; return query(cur * 2, cL, m, l, r) + query(cur * 2 + 1, m + 1, cR, l, r);
    }
    ...
    void init(int size) { N = size; fill(A + 1, A + N, 0); build(1, 1, size); }
    void update(int l, int r, type val) { update(1, 1, N, l, r, make_pair((1 - l) * val, val)); }
    type query(int l, int r) { Data q = query(1, 1, N, l, r); return q.first + q.second; }
};
