...
// Size Balanced Binary Search Tree (no duplicates allowed) with Parallel Arrays
// Time Complexity:
...
template <class Key, class Value, class Comparator = less<Key>> struct SBTArray {
    ...
    int put(int x, const Key &key, const Value &val) {
        if (!x) {
            ...
            KEY.push_back(key); VAL.push_back(val); SZ.push_back(1); L.push_back(0); R.push_back(0); return int(KEY.size()) - 1;
        }
    ...
    SBTArray() : SZ(1, 0), L(1, 0), R(1, 0) {}
    void clear() { root = 0; KEY.clear(); VAL.clear(); SZ.clear(); L.clear(); R.clear(); SZ.push_back(0); L.push_back(0); R.push_back(0); }
