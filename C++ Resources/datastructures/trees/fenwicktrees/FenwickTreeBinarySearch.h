...
template <const int MAXN, class T, const bool ONE_INDEXED> struct FenwickTreeBinarySearch {
    ...
    // returns the first index with prefix sum greater than or equal to value or N + 1 - !ONE_INDEXED if none exists 
    ...
    // returns the first index with prefix sum greater than value or N + 1 - !ONE_INDEXED if none exists
