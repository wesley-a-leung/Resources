...
// Time Complexity:
//   init: O(N)
//   update, rsq(i): O(log N)
//   rsq(l, r): O((log N)^2)
// Memory Complexity: O(N)
template <const int MAXN, class T, const bool ONE_INDEXED> struct FenwickTreeMaxPointRange {
    T A[MAXN], BIT[MAXN], NEG_INF;
    FenwickTreeMaxPointRange(T NEG_INF) : NEG_INF(NEG_INF) { fill(A, A + MAXN, NEG_INF); fill(BIT, BIT + MAXN, NEG_INF); }
