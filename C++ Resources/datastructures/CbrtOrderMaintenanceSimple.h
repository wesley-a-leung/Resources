...
//   rebuild: O(cbrt(N))
//   empty, size: O(1)
//   floor, ceiling, above, below, contains, count: O(cbrt(N) + log(N)) amortized
//   values: O(N)
// Memory Complexity: O(N)
template <class Value, class Comparator = less<Value>> struct CbrtOrderMaintenanceSimple {
    Comparator cmp; double SCALE_FACTOR; vector<Value> small, medium, large;
    CbrtOrderMaintenanceSimple(const double SCALE_FACTOR = 1) : SCALE_FACTOR(SCALE_FACTOR) {}
    template <class It> CbrtOrderMaintenanceSimple(const It st, const It en, const double SCALE_FACTOR = 1) :
