...
//   query: O(B + N / B)
// Memory Complexity: O(N + sqrt N)
template <const int MAXN, const int BLOCKSZ, const bool ONE_INDEXED> struct SqrtDecompArrayLazy {
    ...
    int N; Data small[MAXN], large[MAXN / BLOCKSZ + 5]; Lazy lz[MAXN / BLOCKSZ + 5];
    template <class It> void init(It st, It en) {
        N = en - st; fill(large, large + (N - 1) / BLOCKSZ + 1, qdef); fill(lz, lz + (N - 1) / BLOCKSZ + 1, ldef);
        for (int i = 0; i < N; i++) { small[i] = *(st + i); int bl = i / BLOCKSZ; large[bl] = merge(large[bl], small[i]); }
    }
    void init(int size) {
        N = size; fill(large, large + (N - 1) / BLOCKSZ + 1, qdef); fill(lz, lz + (N - 1) / BLOCKSZ + 1, ldef);
        for (int i = 0; i < N; i++) { small[i] = vdef; int bl = i / BLOCKSZ; large[bl] = merge(large[bl], small[i]); }
    }
    void propagate(int i) {
        int bl = i / BLOCKSZ;
        if (lz[bl] == ldef) return;
        for (int j = bl * BLOCKSZ, en = min(j + BLOCKSZ, N); j < en; j++) small[j] = applyLazy(small[j], lz[bl]);
        lz[bl] = ldef;
    }
    void pushup(int i) {
        int bl = i / BLOCKSZ;
    ...
    void update(int l, int r, const Lazy &v) {
        int l0 = l -= ONE_INDEXED, r0 = r -= ONE_INDEXED; propagate(l0); propagate(r0); Lazy blv = getSegmentVal(v, BLOCKSZ);
        for (; l % BLOCKSZ && l <= r; l++) small[l] = applyLazy(small[l], v);
        for (int bl = l / BLOCKSZ; l + BLOCKSZ - 1 <= r; l += BLOCKSZ, bl++) { large[bl] = applyLazy(large[bl], blv); lz[bl] = mergeLazy(lz[bl], v); }
        for (; l <= r; l++) small[l] = applyLazy(small[l], v);
        pushup(l0); pushup(r0);
    }
    Data query(int l, int r) {
        propagate(l -= ONE_INDEXED); propagate(r -= ONE_INDEXED); Data ret = qdef;
        for (; l % BLOCKSZ && l <= r; l++) ret = merge(ret, small[l]);
        for (; l + BLOCKSZ - 1 <= r; l += BLOCKSZ) ret = merge(ret, large[l / BLOCKSZ]);
        for (; l <= r; l++) ret = merge(ret, small[l]);
        return ret;
    }
