...
template <class T> struct ManberMyersSuffixArray {
    ...
    template <class It> ManberMyersSuffixArray(It st, It en) : N(en - st), S(st, en), ind(N + 1), rnk(N + 1), LCP(N) {
        vector<T> SS = S; sort(SS.begin(), SS.end()); rnk[ind[N] = N] = -1; vector<int> tmp(N + 1, 0);
        for (int i = 0; i < N; i++) { rnk[i] = lower_bound(SS.begin(), SS.end(), S[i]) - SS.begin(); ind[rnk[i] + tmp[rnk[i]]++] = i; }
        for (int h = 1; h < N; h += h) for (int r = 1, cnt = 0; r <= N; r++) {
            if (rnk[ind[r - 1]] == rnk[ind[r]]) cnt++;
            else if (cnt > 0) {
                int l = r - cnt - 1; cnt = 0; sort(ind.begin() + l, ind.begin() + r, [&] (int a, int b) { return rnk[h + a] < rnk[h + b]; });
        ...
        for (int i = 0, k = 0; i < N; i++) {
            ...
            for (int j = ind[rnk[i] + 1]; i + k < N && j + k < N && S[i + k] == S[j + k]; k++);
            if ((LCP[rnk[i]] = k) > 0) k--;
        }
    }
};
...
// Indices are 0-indexed
// lcsRnk(i, j) computes the longest common substring of the ith and jth lexicographically least suffixes
// lcs(i, j) computes the longest common substring of the prefixes starting from index i and j
// In practice, constructor has a small constant, lcs has a moderate constant, still faster than using segment trees
// Time Complexity:
...
template <class T> struct LongestCommonSubstring {
    ...
    template <class It> LongestCommonSubstring(It st, It en) : SA(st, en),
        ST(SA.LCP.begin(), SA.LCP.end(), [&] (int a, int b) { return min(a, b); }) {}
