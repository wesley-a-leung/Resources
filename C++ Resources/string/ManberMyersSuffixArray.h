...
template <const int MAXS, const int ALPHABET_SIZE, const int OFFSET> struct ManberMyersSuffixArray {
    int N, ind[MAXS], rnk[MAXS], temp[MAXS], freq[ALPHABET_SIZE], LCP[MAXS]; string S;
    void run(const string &s) {
        S = s; N = S.length(); ind[N] = N; rnk[N] = -1; fill(freq, freq + ALPHABET_SIZE, 0);
        for (int i = 0; i < N; i++) if (S[i] - OFFSET + 1 != ALPHABET_SIZE) freq[S[i] - OFFSET + 1]++;
        for (int i = 1; i < ALPHABET_SIZE; i++) freq[i] += freq[i - 1];
        for (int i = 0; i < N; i++) rnk[i] = freq[S[i] - OFFSET];
        for (int i = 0; i < N; i++) ind[freq[S[i] - OFFSET]++] = i;
