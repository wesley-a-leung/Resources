    ...
    void solve(int V) {
        ...
        for (int l = 0; l < Q; l += BLOCKSZ) {
            ...
            for (auto &&p : large) if (flag[p.second] != curFlag)
                if (join(1, edges[p.second].v, edges[p.second].w)) ind[cnt++] = p.second;
            stamp[1]++; forest = 0;
            for (int i = 0; i < cnt; i++) {
                if (join(0, edges[ind[i]].v, edges[ind[i]].w)) {
                    join(1, edges[ind[i]].v, edges[ind[i]].w);
            ...
            for (int i = l; i <= r; i++) {
                ...
                stamp[0]++; mst = 0;
                for (auto &&p: small) if (join(0, find(1, edges[p.second].v), find(1, edges[p.second].w)))
