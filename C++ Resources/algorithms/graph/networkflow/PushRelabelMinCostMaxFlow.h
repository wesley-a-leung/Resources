...
template <const int MAXV, class flowUnit, class costUnit, const int SCALE = 2> struct PushRelabelMinCostMaxFlow {
    ...
    int cnt[MAXV * 2], stk[MAXV]; flowUnit FLOW_EPS, maxFlow, ex[MAXV]; costUnit COST_INF, h[MAXV], bnd, minCost;
    ...
    void init(int V) { bnd = 0; for (int i = 0; i < V; i++) adj[i].clear(); }
                ...
                while (ex[v] > FLOW_EPS) {
                    ...
                    for (; cur[v] < adj[v].end(); cur[v]++) {
                        ...
                        if (h[v] + cur[v]->cost - h[cur[v]->to] < 0) {
                            if (lookAhead(cur[v]->to)) { cur[v]--; continue; }
                            push(v, *cur[v], ex[v]);
