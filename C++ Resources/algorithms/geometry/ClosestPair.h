...
template <const int MAXN> struct ClosestPair {
    Point P[MAXN], pointsByY[MAXN], aux[MAXN], best1, best2; F bestDist;
    ...
    F closest(int lo, int hi) {
        if (hi <= lo) return numeric_limits<F>::infinity();
        int mid = lo + (hi - lo) / 2; Point median = P[mid];
        F delta = min(closest(lo, mid), closest(mid + 1, hi)); merge(lo, mid, hi); int m = 0;
        for (int i = lo; i <= hi; i++) if (abs(pointsByY[i].x - median.x) < delta) aux[m++] = pointsByY[i];
        for (int i = 0; i < m; i++) {
            for (int j = i + 1; (j < m) && (aux[j].y - aux[i].y < delta); j++) {
                F distance = aux[i].distanceTo(aux[j]);
                if (distance < delta) {
                    delta = distance;
