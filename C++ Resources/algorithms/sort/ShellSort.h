...
template <class It> void shell_sort(It st, It en) {
    ...
    for (; h >= 1; h = h * 4 / 9) for (int i = h; i < n; i++) for (int j = i; j >= h && st[j] < st[j - h]; j -= h) swap(st[j], st[j - h]);
}
...
template <class It, class Comparator> void shell_sort(It st, It en, Comparator cmp) {
    int n = en - st, h = 1;
    while (h < n * 4 / 9) h = h * 9 / 4 + 1;
    for (; h >= 1; h = h * 4 / 9) for (int i = h; i < n; i++) for (int j = i; j >= h && cmp(st[j], st[j - h]); j -= h) swap(st[j], st[j - h]);
}
